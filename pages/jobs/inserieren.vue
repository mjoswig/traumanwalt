      ...
      <div class="grid grid-cols lg:grid-cols-2 gap-6 md:gap-8 w-full">
        ...
        <div class="flex flex-col space-y-4">
          ...
          <fieldset>
            ...
            <p>Ihre Anzeige ist für <b>60 Tage</b> online und kostet <b>499,00 € zzgl. MwSt.</b></p>
          </fieldset>
        </div>
      </div>
      <fieldset class="flex justify-end">
        <Btn :is-disabled="!canPublish" :is-loading="isPublishing" @click="publishJob">Jetzt veröffentlichen – 499,00 €</Btn>
      </fieldset>
    </form>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async publishJob() {
      ...
      const slugValue = `${req.body.title} ${req.body.company_name}`
      const jobSlug = slugify(slugValue.replace('/', ' '), {
        remove: /[*+~.()'"!:@]/g,
        lower: true,
        locale: 'de'
      })
      ...
      const response = await this.$axios.$post(`/api/stripe/job-posting/pay`, {
        job_id: job.id
      })
