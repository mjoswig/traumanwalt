<template>
  <div>
    ...
    <div v-if="conversations.length" class="flex flex-col space-y-4">
      <nuxt-link :to="`/konto/nachrichten/${conversation.id}`" class="text-gray-700 hover:text-gray-700 hover:no-underline" :class="{ 'font-bold': hasUnreadMessages(conversation) }" v-for="(conversation, index) in conversations" :key="index">
        <AccountSection class="hover:bg-gray-100 flex flex-col space-y-2 lg:flex-row lg:space-y-0 w-full">
          <div class="text-sm flex items-center justify-between space-x-4 w-full lg:hidden">
            <svg v-if="hasUnreadMessages(conversation)" xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" class="bi bi-envelope" viewBox="0 0 16 16">
              <path d="M0 4a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V4Zm2-1a1 1 0 0 0-1 1v.217l7 4.2 7-4.2V4a1 1 0 0 0-1-1H2Zm13 2.383-4.708 2.825L15 11.105V5.383Zm-.034 6.876-5.64-3.471L8 9.583l-1.326-.795-5.64 3.47A1 1 0 0 0 2 13h12a1 1 0 0 0 .966-.741ZM1 11.105l4.708-2.897L1 5.383v5.722Z"/>
            </svg>
            <svg v-if="!hasUnreadMessages(conversation)" xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" class="bi bi-envelope-open" viewBox="0 0 16 16">
          ...
          <div class="flex items-center w-full lg:w-1/3">
            <svg v-if="hasUnreadMessages(conversation)" xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="currentColor" class="bi bi-envelope hidden lg:block lg:mr-4" viewBox="0 0 16 16">
              <path d="M0 4a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V4Zm2-1a1 1 0 0 0-1 1v.217l7 4.2 7-4.2V4a1 1 0 0 0-1-1H2Zm13 2.383-4.708 2.825L15 11.105V5.383Zm-.034 6.876-5.64-3.471L8 9.583l-1.326-.795-5.64 3.47A1 1 0 0 0 2 13h12a1 1 0 0 0 .966-.741ZM1 11.105l4.708-2.897L1 5.383v5.722Z"/>
            </svg>
            <svg v-if="!hasUnreadMessages(conversation)" xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="currentColor" class="bi bi-envelope-open hidden lg:block lg:mr-4" viewBox="0 0 16 16">
              <path d="M8.47 1.318a1 1 0 0 0-.94 0l-6 3.2A1 1 0 0 0 1 5.4v.817l5.75 3.45L8 8.917l1.25.75L15 6.217V5.4a1 1 0 0 0-.53-.882l-6-3.2ZM15 7.383l-4.778 2.867L15 13.117V7.383Zm-.035 6.88L8 10.082l-6.965 4.18A1 1 0 0 0 2 15h12a1 1 0 0 0 .965-.738ZM1 13.116l4.778-2.867L1 7.383v5.734ZM7.059.435a2 2 0 0 1 1.882 0l6 3.2A2 2 0 0 1 16 5.4V14a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V5.4a2 2 0 0 1 1.059-1.765l6-3.2Z"/>
            </svg>
            <span><span class="lg:hidden">„{{ conversation.subject }}“ {{ isClient ? 'an' : 'von' }} </span>{{ isClient ? conversation.to_first_name : conversation.from_first_name }} {{ isClient ? conversation.to_last_name : conversation.from_last_name }}</span>
          </div>
...
export default {
  ...
  methods: {
    hasUnreadMessages(conversation) {
      return this.isClient ? conversation.from_unread_messages : conversation.to_unread_messages
    },
    async loadMore() {
      this.page++
